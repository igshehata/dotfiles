...
# ============================================================================
# Phase 3: Render nix-config from chezmoi templates
# ============================================================================
phase_3() {
    info "Phase 3: Render nix-config from chezmoi templates"
    ...
    mkdir -p "$NIX_CONFIG"
    local chezmoi_cmd=(chezmoi -S "$CHEZMOI_SOURCE")
    if ! command -v chezmoi &>/dev/null; then
        chezmoi_cmd=(nix shell nixpkgs#chezmoi --command chezmoi -S "$CHEZMOI_SOURCE")
    fi

    info "Rendering nix-config from chezmoi source..."
    if ! "${chezmoi_cmd[@]}" cat "$NIX_CONFIG/configuration.nix" > "$NIX_CONFIG/configuration.nix"; then
        fail "Failed to render $NIX_CONFIG/configuration.nix"
        return 1
    fi

    if ! "${chezmoi_cmd[@]}" cat "$NIX_CONFIG/flake.nix" > "$NIX_CONFIG/flake.nix"; then
        fail "Failed to render $NIX_CONFIG/flake.nix"
        return 1
    fi
    ...
    if ! grep -q "user = \"$CURRENT_USER\"" "$config_nix" || \
       ! grep -q "users.users.\"$CURRENT_USER\"" "$config_nix" || \
       ! grep -q "system.primaryUser = \"$CURRENT_USER\"" "$config_nix"; then
        fail "Rendered configuration.nix does not match current user $CURRENT_USER"
        return 1
    fi

    if ! grep -q "darwinConfigurations.\"$HOSTNAME\"" "$flake_nix"; then
        fail "Rendered flake.nix does not match current host $HOSTNAME"
        return 1
    fi

    success "Rendered nix-config for $CURRENT_USER@$HOSTNAME"
...
phase_4() {
    ...
    if [[ $has_brew -eq 1 ]] && brew list --cask raycast &>/dev/null; then
        info "Raycast state is not synced via dotfiles. Restore it from your private 'Export Settings & Data' bundle after install."
    fi
...
# ============================================================================
# Phase 8: Post-apply machine setup
# ============================================================================
phase_8() {
    info "Phase 8: Post-apply machine setup"
    skip "Handled by .chezmoiscripts during chezmoi apply"
}
