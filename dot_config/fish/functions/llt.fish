function llt --description 'Detailed tree view (level 2) with collapsed dependency dirs'
    ...
    if test (count $found) -gt 0
        ...
        eza -lT --no-permissions --icons --level=2 --git -I "$ignore_glob" $pass_args
        # Print collapsed dirs as a flat list at tree root level
        for d in $found
            echo "├── $d/"(set_color brblack)" [collapsed]"(set_color normal)
        end
    else
        eza -lT --no-permissions --icons --level=2 --git $pass_args
    end
end
