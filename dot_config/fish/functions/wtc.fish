function wtc -d "Create worktree + tmux window with Claude and dev server"
    if test (count $argv) -lt 1
        echo "Usage: wtc <branch> [wt-switch-args...] [-- prompt...]" >&2
        return 1
    end
    ...
    set -l bootstrap_args wtx claude $branch $wt_args -- $prompt
